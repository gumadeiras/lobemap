...
OUT_DIR = DATASETS_DIR / "reference-tables"
...
def read_grabe_pn_expression() -> pd.DataFrame:
    path = DATASETS_DIR / "grabe-2015/data/source/grabe_2015_pn_expression.csv"
    if not path.exists():
        return pd.DataFrame()
...
def read_grabe_sensory_line_expression() -> pd.DataFrame:
    path = DATASETS_DIR / "grabe-2015/data/source/grabe_2015_sensory_line_expression.csv"
    if not path.exists():
        return pd.DataFrame()
    return pd.read_csv(path).fillna("").replace(r"^\s+$", "", regex=True)
...
def read_benton_table() -> pd.DataFrame:
    path = DATASETS_DIR / "benton-2025/data/source/44319_2025_476_MOESM2_ESM.xlsx"
    if not path.exists():
...
def read_bates_schlegel_names() -> set[str]:
    ...
    html = DATASETS_DIR / "bates-schlegel-2020/data/source/glomeruli_atlas_interactive.html"
    return {
...
def source_names() -> dict[str, set[str]]:
    sources = {
        "grabe_2015": set(
            pd.read_csv(DATASETS_DIR / "grabe-2015/data/derived/al_atlas_materials.csv")[
                "short_name"
            ]
        ),
        "hemibrain": set(
            pd.read_csv(DATASETS_DIR / "hemibrain/data/source/hemibrain_al_microns_materials.csv")[
                "name"
            ]
        ),
        "flywire": set(
            pd.read_csv(DATASETS_DIR / "flywire/data/source/flywire_al_materials.csv")["name"]
        ),
        "door_map": set(
            pd.read_csv(DATASETS_DIR / "door/data/source/door_al_map_glomeruli.csv")[
                "glomerulus"
            ]
        ),
        "door_mappings": set(
            pd.read_csv(DATASETS_DIR / "door/data/source/door_mappings.csv")["glomerulus"]
        ),
...
def pn_line_source(rows: pd.DataFrame) -> str:
    sources = []
    if not rows.empty and (rows["gh146_gal4"] == "positive").any():
        sources.append("Grabe 2015 Table 1")
    if not rows.empty and (rows["chat_gal4"] == "positive").any():
        sources.append("Grabe 2016 Table S2")
    return "; ".join(sources)
...
def main() -> None:
    ...
    door = pd.read_csv(DATASETS_DIR / "door/data/source/door_mappings.csv").fillna("")
    scenes = pd.read_csv(DATASETS_DIR / "hemibrain/data/source/odour_scenes.csv").fillna("")
    vfb = pd.read_csv(DATASETS_DIR / "hemibrain/data/source/vfb_glomerulus_terms.csv").fillna("")
