...
materials <- nat::materials(flywire_al.surf)
materials$name <- row.names(materials)
row.names(materials) <- NULL
write.csv(
  materials[, c("id", "name", "col")],
  file.path(source_dir, "flywire_al_materials.csv"),
  row.names = FALSE,
  quote = TRUE
)

vertices <- flywire_al.surf$Vertices[, c("PointNo", "X", "Y", "Z")]
write.csv(
  vertices,
  gzfile(file.path(source_dir, "flywire_al_vertices.csv.gz")),
  row.names = FALSE,
  quote = FALSE
)

face_chunks <- lapply(seq_along(flywire_al.surf$Regions), function(index) {
  faces <- flywire_al.surf$Regions[[index]]
  data.frame(
...
write.csv(
  ...
  gzfile(file.path(source_dir, "flywire_al_faces.csv.gz")),
  row.names = FALSE,
  quote = TRUE
)
